...
import com.pomodoro.form.ArticleForm;
...
public class ArticleController {
    ...
    // 新規投稿画面を表示
    @GetMapping("/new")
    public String getPost(@ModelAttribute ArticleForm form) {
        return "circles/new";
    }
    // 新規投稿処理
    @PostMapping("/new")
    public String postRequest(@ModelAttribute ArticleForm form) {
        //ログを表示
    ...
    // 編集画面を表示
    @GetMapping("/edit/{id}")
    public String editPost(@ModelAttribute ArticleForm form,Model model,@PathVariable("id") int id) {
        MArticle article = articleservice.getArticleOne(id);
        form = modelMapper.map(article, ArticleForm.class);
    ...
    public String deleteArticle(ArticleForm form, Model model) {
